...
def close_popups(page):
    """Sık çıkan popup pencerelerini kapat"""
    try:
        # 1. Overlay (#imgPopupOverlay) - Genelde büyük bir reklam/bilgi popup'ı
        overlay = page.locator('#imgPopupOverlay')
        if overlay.is_visible():
            # Kapatma butonu genellikle bir X simgesi veya dışarıya tıklama ile olur
            # Genelde butonun ID'si veya class'ı olur. Bu sitede overlay'in kendisine tıklamak yetebilir
            # veya içindeki kapatma butonunu arayalım.
            close_btn = page.locator('#imgPopupOverlay .close, #imgPopupOverlay [onclick*="close"], #imgPopupOverlay .cls-btn')
            if close_btn.count() > 0 and close_btn.first.is_visible():
                close_btn.first.click()
                print("[CF] Popup kapatıldı (buton ile)", flush=True)
            else:
                # Buton bulunamazsa overlay'in köşesine veya dışına tıkla
                page.mouse.click(10, 10)
                print("[CF] Popup kapatılmaya çalışıldı (pixel tıklama)", flush=True)
            time.sleep(1)
        
        # 2. Diğer olası modal/bilgi kutuları
        modals = page.locator('.modal.show, .bootbox.modal')
        if modals.count() > 0:
            for i in range(modals.count()):
                if modals.nth(i).is_visible():
                    # Kapatma butonu
                    modals.nth(i).locator('btn-close, .close, [data-dismiss="modal"]').first.click()
                    print(f"[CF] Modal {i+1} kapatıldı", flush=True)
    except Exception as e:
...
def wait_for_cloudflare(page, timeout=45000):
    ...
    def simulate_human_behavior():
        try:
            # Popup'ları her adımda kontrol et
            close_popups(page)
    ...
    try:
        ...
        if ilan_count > 0:
            print(f"[CF] İlanlar zaten yüklü ({ilan_count} adet), devam ediliyor", flush=True)
            return True
    except Exception as e:
        pass
...
def fetch_listings_playwright():
    ...
    with sync_playwright() as p:
        ...
        # === BAŞLANGIÇ SAYFASINA GİT ===
        try:
            page.goto(URL, timeout=90000, wait_until="networkidle")
            if not wait_for_cloudflare(page):
                print("[PLAYWRIGHT] Cloudflare challenge geçilemedi", flush=True)
                browser.close()
                ACTIVE_SCAN = False
                return (None, "Cloudflare bypass failed")
        except Exception as e:
            print(f"[PLAYWRIGHT] Sayfa yükleme hatası: {e}", flush=True)
            browser.close()
            ACTIVE_SCAN = False
            return (None, str(e))
        ...
        while True:
            ...
            page_num += 1
            print(f"[PLAYWRIGHT SAYFA {page_num}] Taranıyor...", flush=True)

            # Popup'ları her sayfada kontrol et
            close_popups(page)

            # 1. AJAX yüklemesinin tamamlanmasını bekle
            old_pagination_text = ""
            try:
                # KRİTİK: Üstteki bilgi güncellenmiyor, alttakini kullan (last() veya specific selector)
                pagination_info = page.locator('.cb-list-info, .pagination-info, .summary').last
                if pagination_info.count() > 0:
                    old_pagination_text = pagination_info.inner_text()
            except:
                pass
            ...
            try:
                ...
                page.wait_for_selector('a[href*="/ilan/"]', timeout=20000)
                page.wait_for_timeout(2000) # Render payı
            except:
                if page_num > 1:
                    print(f"[PLAYWRIGHT SAYFA {page_num}] İlan bulunamadı, tarama bitti", flush=True)
                    break
                else:
                    print("[PLAYWRIGHT] İlk sayfada ilan bulunamadı!", flush=True)
                    continue
            ...
            listings = page.evaluate(
                ...
                // Tüm ilan kartlarını veya linklerini bul
                const items = document.querySelectorAll('.cb-list-items, [class*="listing-item"], .card');
                
                if (items.length === 0) {
                   // Fallback: Doğrudan linklerden git
                   document.querySelectorAll('a[href*="/ilan/"]').forEach(a => {
                       ...
                       seen.add(kod);
                       out.push({ kod: kod, fiyat: "Bilinmiyor", title: "İlan " + kod, link: href });
                   });
                   return out;
                }
                ...
                items.forEach(el => {
                    ...
                    let fiyat = "Fiyat yok";
                    const priceElements = Array.from(el.querySelectorAll('span, div, b, strong, p'))
                        .filter(e => e.innerText.includes('TL') || e.innerText.includes('₺'))
                        .filter(e => /[\\d\\.,]+/.test(e.innerText));
                    
                    if (priceElements.length > 0) {
                        const bestMatch = priceElements.find(e => e.className.includes('price') || e.className.includes('h5')) || priceElements[0];
                        fiyat = bestMatch.innerText.trim();
                    }

                    let title = "";
                    const titleEl = el.querySelector('h2, h3, .title, .card-title');
                    if (titleEl) {
                        title = titleEl.innerText.trim().replace(/\\s*-?\\s*ML-\\d+-\\d+\\s*$/i, '');
                    }
                    ...
                    out.push({ kod: kod, fiyat: fiyat, title: title, link: fullHref });
                });
            ...
            if not listings:
                print(f"[PLAYWRIGHT SAYFA {page_num}] Liste boş, tarama bitti", flush=True)
                break
            ...
            if page_num % 10 == 0:
                send_message(f"🔄 <b>TARAMA DEVAM EDİYOR</b>\n\n📄 Sayfa: {page_num}\n📊 Toplam ilan: {len(results)}")

            # 3. Sonraki Sayfaya Tıkla (AJAX)
            try:
                # Sitede "Sonraki" butonu yok, sayfa numaralarına tıklamalıyız.
                # Mevcut aktif sayfanın bir fazlasını ara.
                target_page = page_num + 1
                next_button = page.locator(f'.pagination a[href*="pager_p={target_page}"], .pagination a:has-text("{target_page}")').first
                
                if next_button.count() > 0 and next_button.is_visible():
                    print(f"[PLAYWRIGHT] Sayfa {target_page}'e tıklanıyor (AJAX)...", flush=True)
                    close_popups(page)
                    next_button.scroll_into_view_if_needed()
                    next_button.click()
                    
                    # AJAX Bekle: Alt bilgi metninin değişimini bekle
                    if old_pagination_text:
                        try:
                            # Metnin değişmesini veya yeni sayfa numarasının belirmesini bekle
                            page.wait_for_function(
                                f"() => document.querySelectorAll('.cb-list-info, .pagination-info, .summary')[1]?.innerText !== '{old_pagination_text}'",
                                timeout=15000
                            )
                        except:
                    ...
                    print(f"[PLAYWRIGHT] Sayfa {target_page} butonu bulunamadı, tarama bitiriliyor.", flush=True)
                    break
            except Exception as e:
                print(f"[PLAYWRIGHT] Navigasyon hatası: {e}", flush=True)
                break

            if page_num >= 100:
                print("[PLAYWRIGHT] 100 sayfa limitine ulaşıldı.", flush=True)
                break
...
def run_scan_with_timeout():
    ...
    # === KORUMA: Minimum ilan oranı kontrolü ve 400 ilan sınırı ===
    existing_count = len(state.get("items", {}))
    
    # Kural 1: Eğer 400'den az ilan varsa (ve site aslında çok daha fazlasına sahipse) korumaya gir
    # Bot bazen 58 sayfa varken 35'te kesiyor hatasını çözmek için:
    if len(listings) < 400 and existing_count > 450:
        msg = "⚠️ <b>KORUMA: Yetersiz İlan Sayısı</b>\n\n"
        msg += f"📊 Bellekte: <b>{existing_count}</b> ilan\n"
        msg += f"🔍 Taramada bulunan: <b>{len(listings)}</b> ilan\n"
        msg += "🛡️ Beklenen minimum: <b>400</b> ilan\n\n"
        msg += "❌ <b>Durum:</b> Tarama erken kesilmiş olabilir, iptal edildi. ✅"
        send_message(msg)
        print(f"[KORUMA] Yetersiz ilan: {len(listings)} (min: 400)", flush=True)
    ...
    # Kural 2: Mevcut ilanların en az %40'ı bulunmalı (Ratio koruması)
    if not is_first_run and existing_count > 100:
        min_expected = int(existing_count * MIN_LISTING_RATIO)
        if len(listings) < min_expected:
            msg = "⚠️ <b>KORUMA: Anormal Tarama Oranı</b>\n\n"
            msg += f"📊 Bellekte: <b>{existing_count}</b> ilan\n"
            msg += f"🔍 Taramada bulunan: <b>{len(listings)}</b> ilan\n"
            msg += f"🛡️ Min beklenen: <b>{min_expected}</b> (%{int(MIN_LISTING_RATIO*100)})\n\n"
            msg += "❌ <b>Durum:</b> Veritabanı korunarak tarama iptal edildi. ✅"
            send_message(msg)
